...
Globe.ECKey = (function () {
	var ECDSA = Globe.ECDSA;
	var ecparams = EllipticCurve.getSECCurveByName("secp256k1");
	var rng = new SecureRandom();
	...
	var ECKey = function (input) {
		if (!input) {
			...
			this.priv = input;
		} else if (Globe.Util.isArray(input)) {
	...
	ECKey.prototype.getPubKeyHash = function () {
		...
		return this.pubKeyHash = Globe.Util.sha256ripe160(this.getPub());
	};

	ECKey.prototype.getPubKeyHashCompressed = function () {
		if (this.pubKeyHashCompressed) return this.pubKeyHashCompressed;
		return this.pubKeyHashCompressed = Globe.Util.sha256ripe160(this.getPubCompressed());
	};

	ECKey.prototype.getGlobeAddress = function () {
		var hash = this.getPubKeyHash();
		var addr = new Globe.Address(hash);
		return addr.toString();
	};

	ECKey.prototype.getGlobeAddressCompressed = function () {
	...
	ECKey.prototype.getGlobeWalletImportFormat = function () {
		var bytes = this.getGlobePrivateKeyByteArray();
		bytes.unshift(ECKey.privateKeyPrefix); // prepend 0x80 byte
		var checksum = Globe.Util.dsha256(bytes);
		bytes = bytes.concat(checksum.slice(0, 4));
		var privWif = Globe.Base58.encode(bytes);
		return privWif;
	};

	// Sipa Private Key Wallet Import Format Compressed
	ECKey.prototype.getGlobeWalletImportFormatCompressed = function () {
		var bytes = this.getGlobePrivateKeyByteArray();
		bytes.unshift(ECKey.privateKeyPrefix); // prepend 0x80 byte	
		bytes.push(0x01);    // append 0x01 byte for compressed format
		var checksum = Globe.Util.dsha256(bytes);
		bytes = bytes.concat(checksum.slice(0, 4));
		var privWifComp = Globe.Base58.encode(bytes);
		return privWifComp;
	};

	// Private Key Hex Format 
	ECKey.prototype.getGlobeHexFormat = function () {
		return Crypto.util.bytesToHex(this.getGlobePrivateKeyByteArray()).toString().toUpperCase();
	};

	// Private Key Base64 Format 
	ECKey.prototype.getGlobeBase64Format = function () {
		return Crypto.util.bytesToBase64(this.getGlobePrivateKeyByteArray());
	};

	ECKey.prototype.getGlobePrivateKeyByteArray = function () {
	...
	ECKey.prototype.toString = function (format) {
		...
		if (format.toString().toLowerCase() == "base64" || format.toString().toLowerCase() == "b64") {
			return this.getGlobeBase64Format();
		}
		// Wallet Import Format
		else if (format.toString().toLowerCase() == "wif") {
			return this.getGlobeWalletImportFormat();
		}
		else if (format.toString().toLowerCase() == "wifcomp") {
			return this.getGlobeWalletImportFormatCompressed();
		}
		else {
			return this.getGlobeHexFormat();
		}
	};
