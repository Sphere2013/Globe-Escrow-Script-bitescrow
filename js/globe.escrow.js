Globe.Escrow = {
	...
	CreateEscrowPair: function() {
		...
		var Kx = new Globe.ECKey(x);
		var Ky = new Globe.ECKey(y);
		...
		var hashGxy = Globe.Util.dsha256(Gxy.getEncoded());
		...
		invitationA = invitationA.concat(Globe.Util.dsha256(invitationA).slice(0,4));
		invitationB = invitationB.concat(Globe.Util.dsha256(invitationB).slice(0,4));

		return {
			invitationA: Globe.Base58.encode(invitationA),
			invitationB: Globe.Base58.encode(invitationB)
		}
	},
	...
	CreatePaymentCode: function(thecode) {
		...
		var thecodeBytes = Globe.Base58.decode(thecode);
		...
		var hash160 = Globe.Util.sha256ripe160(Gxyz.getEncoded());
		var address = new Globe.Address(hash160).toString();
		...
		invitationP = invitationP.concat(Globe.Util.dsha256(invitationP).slice(0,4));
		...
		confirmationP = confirmationP.concat(Globe.Util.dsha256(confirmationP).slice(0,4));



		return {
			address: address,
			invitationP: Globe.Base58.encode(invitationP),
			confirmationP: Globe.Base58.encode(confirmationP)
		}
	...
	VerifyPaymentCode: function(code1, code2) {
		...
		var thecodeBytes = Globe.Base58.decode(escrowInvitationCode);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
		...
		thecodeBytes = Globe.Base58.decode(paymentInvitationCode);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
		...
		for (var i = 0; i < 20; i++) {
			if (hash160[i] != thecodeBytes[8+1+1+32+i]) { 
				return { result: false, address: '', message: 'Warning: The Hash160 of the Globe Address does not match!' };
			}
		}
	...
	VerifyConfirmationCode: function(code1, code2, code3) {
		...
		var thecodeBytes = Globe.Base58.decode(escrowInvitationCodeA);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
    ...
    var privAi = BigInteger.fromByteArrayUnsigned(privA);
		...
		thecodeBytes = Globe.Base58.decode(escrowInvitationCodeB);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
    ...
    var privBi = BigInteger.fromByteArrayUnsigned(privB);
		...
		thecodeBytes = Globe.Base58.decode(paymentConfirmationCode);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
	...
	Redeem: function(code1, code2, code3) {
	    ...
	    /* Code A */
		thecodeBytes = Globe.Base58.decode(codea);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
		if (thecodeBytes.length != 74) { return null; }
		...
		/* Code B */
		thecodeBytes = Globe.Base58.decode(codeb);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
		if (thecodeBytes.length != 74) { return null; }
		...
		/* Code P */
		thecodeBytes = Globe.Base58.decode(codep);
		thecodeBytes = thecodeBytes.slice(0, thecodeBytes.length - 4);
		if (thecodeBytes.length != 74) { return null; }
		...
		var globe = new Globe.ECKey(xyz.toByteArrayUnsigned());

		return {
			address: globe.getGlobeAddress(),
			wif: globe.getGlobeWalletImportFormat()
		}
	}
}
