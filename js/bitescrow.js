$(function() {
	...
	$('#generate-codepair').click(function(e) {
		...
		var escrow = Globe.Escrow.CreateEscrowPair();
		if (escrow != null) {
	...
	$('#generate-payment').click(function(e) {
		...
		var payment = Globe.Escrow.CreatePaymentCode(code1.val());
		if (payment != null) {
	...
	$('#verify').click(function(e) {
		...
		var verify = Globe.Escrow.VerifyPaymentCode(code1.val(), code2.val());		
		if (verify.result) {
			verification.text('The globe address ' + verify.address + ' is associated with your escrow');
		} else {
	...
	$('#verifyc').click(function(e) {
		...
		var verify = Globe.Escrow.VerifyConfirmationCode(code1.val(), code2.val(), code3.val());		
		if (verify.result) {
			verification.text('The globe address ' + verify.address + ' is associated with the transaction');
		} else {
			verification.text(verify.message);
		}
	...
	$('#redeem').click(function(e) {
		...
		var redeem = Globe.Escrow.Redeem(code1.val(), code2.val(), code3.val());		
		if (redeem != null) {
	...
	if (hash['runtests'] == 'true') {
		...
		// Test known (Casascius Globe Tool generated) invitations
		var test = tests[0];
		var verify = Globe.Escrow.VerifyPaymentCode(test[1], test[2]);
		if (!verify) {
		...
		test = tests[1];
		var verify = Globe.Escrow.VerifyPaymentCode(test[0], test[2]);
		if (!verify) {
		...
		for (var i = 0; i < tests.length; i++) {
			...
			var globe = Globe.Escrow.Redeem(test[0], test[1], test[2]);
			if (globe.address != test[3] || globe.wif != test[4]) {
		...
		for (var i = 0; i < 5; i++) {
			...
			var payment = Globe.Escrow.CreatePaymentCode(escrow.invitationA);
			...
			var verify = Globe.Escrow.VerifyPaymentCode(escrow.invitationB, payment.invitationP);
			if (!verify.result) {
			...
			var verify = Globe.Escrow.VerifyConfirmationCode(escrow.invitationA, escrow.invitationB, payment.confirmationP);
			if (!verify.result) {
			...
			// Redeem the globes
			var globe = Globe.Escrow.Redeem(escrow.invitationA, escrow.invitationB, payment.invitationP);
			if (globe.address != payment.address || globe.wif == '' || globe.wif == null) {
